...
# The layer norms in the encoder and decoder however are moved to preLN location
# The encoder is also replace by a alexatm encoder model which is similar to BERT
# for more info see http://arxiv.org/abs/2002.04745
...
from .alexatm_encoder import AlexaTMEncoder, AlexaTMLayerNorm

from .alexatm_seq2seq_config import AlexaTMSeq2SeqConfig
...
class AlexaTMSeq2SeqLearnedPositionalEmbedding(nn.Embedding):
    """
    This module learns positional embeddings up to a fixed maximum size.
    """

    def __init__(self, num_embeddings: int, embedding_dim: int):
        # make padding idx = 1 for pos embeddings
        padding_idx = 1

        # Similar to BART AlexaTMSeq2Seq is set up so that if padding_idx is specified then offset the embedding ids
        # by 2 and adjust num_embeddings appropriately. Other models dont have this hack
        self.offset = 2
...
class AlexaTMSeq2SeqAttention(nn.Module):
...
class AlexaTMSeq2SeqDecoderLayer(nn.Module):
    def __init__(self, config: AlexaTMSeq2SeqConfig):
        super().__init__()
        self.embed_dim = config.d_model

        self.self_attn = AlexaTMSeq2SeqAttention(
        ...
        self.self_attn_layer_norm = AlexaTMLayerNorm(config.d_model, eps=config.layer_norm_eps
                                                     )
        self.encoder_attn = AlexaTMSeq2SeqAttention(
            self.embed_dim,
            config.decoder_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=True
        )
        self.encoder_attn_layer_norm = AlexaTMLayerNorm(config.d_model, eps=config.layer_norm_eps
                                                        )
        self.fc1 = nn.Linear(self.embed_dim, config.decoder_ffn_dim)
        self.fc2 = nn.Linear(config.decoder_ffn_dim, self.embed_dim)
        self.final_layer_norm = AlexaTMLayerNorm(config.d_model, eps=config.layer_norm_eps
                                                 )
...
class AlexaTMSeq2SeqPretrainedModel(PreTrainedModel):
    config_class = AlexaTMSeq2SeqConfig
...
class AlexaTMSeq2SeqDecoder(AlexaTMSeq2SeqPretrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a :class:`AlexaTMSeq2SeqDecoderLayer`
    ...
    def __init__(self, config: AlexaTMSeq2SeqConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.embed_positions = AlexaTMSeq2SeqLearnedPositionalEmbedding(
            config.max_position_embeddings,
            config.d_model
        )
        self.layers = nn.ModuleList([AlexaTMSeq2SeqDecoderLayer(config) for _ in range(config.decoder_layers)])
        # remove this in preLN
        # self.layernorm_embedding = nn.LayerNorm(config.d_model)
        # add this for preLN
        self.layernorm_output = AlexaTMLayerNorm(config.d_model, eps=config.layer_norm_eps
                                                 )
...
class AlexaTMEncoderForSeq2Seq(AlexaTMSeq2SeqPretrainedModel):
    def __init__(self, config, token_embeddings, is_decoder=False):
        ...
        # this for embeddings following Embeddings
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

        self.pre_ln_encoder = AlexaTMEncoder(self.config)
    ...
    def parallelize(self, num_devices, start_device):
        ...
        self.pre_ln_encoder.parallelize(num_devices, start_device)
...
class AlexaTMSeq2SeqModel(AlexaTMSeq2SeqPretrainedModel):
    def __init__(self, config: AlexaTMSeq2SeqConfig):
        super().__init__(config)
        ...
        self.encoder = AlexaTMEncoderForSeq2Seq(config, self.shared)

        if config.share_embeddings:
            self.decoder = AlexaTMSeq2SeqDecoder(config, self.shared)
        else:
            self.decoder = AlexaTMSeq2SeqDecoder(config)
    ...
    def parallelize(self, num_devices, start_device):
        ...
        if num_devices == 2:
            ...
            self.shared.to(device_list[0])
            self.encoder.parallelize(num_devices//2, start_device)
            self.decoder.parallelize(num_devices//2, start_device + num_devices//2)
    ...
    def forward(
        ...
        # different to other models, AlexaTMSeq2Seq automatically creates decoder_input_ids from
...
class AlexaTMSeq2SeqForConditionalGeneration(AlexaTMSeq2SeqPretrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: AlexaTMSeq2SeqConfig):
        super().__init__(config)
        self.model = AlexaTMSeq2SeqModel(config)
    ...
    def parallelize(self, num_devices=2, start_device=0):
        """
        This function put the model on multiple GPUs allowing to run inference with larger model sizes on devices
        ...
        :param start_device: the starting device id
        :return:
        """
        assert num_devices % 2 == 0, "parallelize only support parallelizing into even number of GPUs"
        assert num_devices + start_device <= torch.cuda.device_count(), "number of available GPUs are less than " \
        ...
        self.model.parallelize(num_devices, start_device)
    ...
    # have to overwrite this to work with model_parallel
    ...
    def _prepare_decoder_input_ids_for_generation(
        self, input_ids: torch.LongTensor, decoder_start_token_id: int = None, bos_token_id: int = None,
            model_kwargs=None,  # added to support transformers 4.11+
            device=None  # added to support transformers 4.11+
    ) -> torch.LongTensor:

        decoder_start_token_id = self._get_decoder_start_token_id(decoder_start_token_id, bos_token_id)
        # added to support transformers 4.11+
        if isinstance(input_ids, int):
            if model_kwargs is not None and "decoder_input_ids" in model_kwargs:
                return model_kwargs.pop("decoder_input_ids")

            # in this case input_ids are actually batch size
            ...
            decoder_input_ids = (
                torch.ones((input_ids.shape[0], 1), dtype=input_ids.dtype,
                           device=self.model.decoder.starting_device)
                * decoder_start_token_id
            )
