...
import os
import torch
from alexa_teacher_models.tokenizers import AlexaTMTokenizerFast
...
def load_model_on_multiple_gpus( model_path, num_gpus=4):
    ...
    tokenizer = AlexaTMTokenizerFast.from_pretrained(model_path,
                                                     vocab_file=os.path.join(model_path, "20b_tokenizer.model"))
...
def download_data():
    dataset = dict()
    langs = {'ara': 'Arabic', 'fra': 'French', 'eng': 'English', 'deu': 'German', 'ita': 'Italian', 'jpn': 'Japanese',
             'hin': 'Hindi', 'mar': 'Marathi', 'tam': 'Tamil', 'tel': 'Telugu', 'spa': 'Spanish'}
...
def xsum(model, tokenizer, device, NSHOTS):
    ...
    for i in range(NSHOTS):
        shots += """Article: {0} ==> Short summary: {1} <br><br><br> """.format(short_examples[i]['document'],
                                                                                short_examples[i]['summary'])
    ...
    save_output(cleaned_summaries, os.path.join(os.getcwd(),
                                                 'xsum/summaries' +
                                                 '_20B_{}shot.txt'.format(NSHOTS)))
...
def flores_101(model, tokenizer, device, NSHOTS):
    ...
    for ls in langs.keys():
        for lt in langs.keys():
            ...
            shots = ''
            for i in range(NSHOTS):
                shots += """Sentence:  {0}; Translation in {1}: {2}; """.format(dataset[ls]['dev'][i]['sentence'],
                                                                                langs[lt],
                                                                                dataset[lt]['dev'][i]['sentence'])

            translated = []
            tgt = []
            for i, d in enumerate(dataset[ls]['devtest']):

                test = """[CLM] """ + shots + """Sentence:  {0}; Translation in {1}:""".format(d['sentence'],
                                                                                               langs[lt])
                if i == 0:
                    print('Example input to the model: \n', test)
            ...
            save_output(cleaned_translated, os.path.join(os.getcwd(),
                                                         'flores101_outputs/' + ls + 'T' + lt +
                                                         '_20B_{}shot.txt'.format(NSHOTS)))
...
def main():
    ...
    assert ((args.number_of_gpus % 2 == 0 or args.number_of_gpus == 1)
            and args.number_of_gpus <= 8), "You can either use 1 GPU or " \
                                           "an even number of GPUs up to 8"

    model, tokenizer, device = load_model_on_multiple_gpus(args.model_path, num_gpus=args.number_of_gpus)
    print('Generating outputs for {} task using AlexaTM 20B using {} shots!'.format(args.task, args.number_of_shots))
    ...
    if args.task == 'xsum':
        ...
        print('Task {} is not supported!'.format(args.task))
